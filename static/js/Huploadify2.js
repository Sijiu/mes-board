...
$.fn.Huploadify = function(opts){
	...
	var myItemTemp = '<tr id="${fileID}" class="uploadify-queue-item">' +
                    '<td class="up-td">${fileName}</td>'+
                    '<td class="up-td">${fileSize}</td>'+
                    '<td class="up-td"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div></td>' +
                    '<td class="up-td"><button type="button" class="btn btn-danger delfilebtn" style="color:#fff">删除</button></td></tr>';
    var defaults = {
		...
		itemTemplate:myItemTemp,//上传队列显示的模板
	...
	this.each(function(){
        ...
        var myUploadFileListStr = '<table class="table table-striped table-bordered table-hover" id="file-table">'+
                                '<thead><tr><th class="up-th" style="width:40%">名称</th><th class="up-th" style="width:15%">大小</th>'+
                                '<th class="up-th" style="width:30%">状态</th><th class="up-th" style="width:15%">动作</th></tr></thead>'+
                                '<tbody id="file_upload_'+instanceNumber+'-queue" class="uploadify-queue"></tbody></table>';
		_this.append(inputStr+myUploadFileListStr);
	  ...
	  fileObj = {
		  ...
		  init: function() {	  
			  ...
			  _this.find('.uploadify-button').on('click',function(){
				  ...
				  _this.find("a").addClass("btn btn-primary")
				  _this.siblings("ul").find("li").removeClass();
				  console.log("start up load,")
				});
	...
	var returnObj =  {
		...
		upload : function(fileIndex){
			if(fileIndex === '*'){
                ...
                fileObj.fileFilter = [];
                if(jQuery("#success-tip").is(":hidden")){
                    jQuery("#success-tip").fadeIn(200).fadeOut(1000);
                }
                jQuery(".uploadify-button").prop("disabled",false);
			}
			else{
